...
class PropertyMixin:
    ...
    def get_customer_custom_properties(self, customer_id: int) -> List[CustomProperty]:
        """
        Get all custom properties for a customer (org unit).

        Uses the org-units/{orgUnitId}/custom-properties endpoint.
        In N-Central, customerId == orgUnitId for customers.
        ...
        Example:
            props = nc.get_customer_custom_properties(237)
            for p in props:
                print(f"{p.propertyName}: {p.value}")
        """
        logger.debug(f"Fetching custom properties for customer {customer_id}")
        response = self.get(f"org-units/{customer_id}/custom-properties")
        if isinstance(response, dict) and "data" in response:
            properties_data = response.get("data", [])
        else:
            properties_data = response if isinstance(response, list) else []
    ...
    def get_customer_custom_property(
        self,
        customer_id: int,
        property_id: int
    ) -> Optional[CustomProperty]:
        """
        Get a specific custom property for a customer.
        ...
        Returns:
            CustomProperty or None

        Raises:
            APIError: If the API request fails
        """
        logger.debug(f"Fetching custom property {property_id} for customer {customer_id}")
        response = self.get(f"org-units/{customer_id}/custom-properties/{property_id}")
        if isinstance(response, dict) and "data" in response:
            prop_data = response.get("data")
        else:
            prop_data = response if response else None
    ...
    def get_customer_custom_property_by_name(
        self,
        customer_id: int,
        property_name: str
    ) -> Optional[CustomProperty]:
        """
        Get a customer custom property by name.
        ...
        Raises:
            APIError: If the API request fails
        """
        logger.debug(f"Searching for custom property '{property_name}' on customer {customer_id}")
        properties = self.get_customer_custom_properties(customer_id)
        for prop in properties:
            if prop.propertyName == property_name:
                return prop
        return None

    def update_customer_custom_property(
        self,
        customer_id: int,
        property_id: int,
        value: str
        ...
        """
        Update a custom property value for a customer.
        ...
        Raises:
            APIError: If the API request fails
        """
        logger.info(f"Updating custom property {property_id} on customer {customer_id}")
        response = self.put(
            f"org-units/{customer_id}/custom-properties/{property_id}",
            {"value": value}
        )
        if isinstance(response, dict) and "data" in response:
            prop_data = response.get("data")
